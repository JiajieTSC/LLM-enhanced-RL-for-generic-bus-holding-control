...
- Agent's state at the end of the action, denoted as `next_state` in the code. Please be aware that the s2 in `next_state` is always the same as the s2 in `current_state` since passenger boarding and alighting is not allowed during holding control.
